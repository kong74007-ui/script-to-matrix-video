...
- Set BGM to `auto` when a configured library has a suitable track and the user has not requested silence, except for `nine-grid-reveal`, which keeps its bound audio. An explicit no-BGM instruction always wins; handle such a request for nine-grid as a separate custom version. With narration, keep music subordinate and enable ducking; without narration, use a restrained full music bed.
- Derive total duration from the copy, semantic scenes, and actual synthesized audio, except for the fixed 12-second `nine-grid-reveal` composition. Do not force a target duration for other modes.
...
- Rendering requires Python 3.10 or newer plus `ffmpeg` and `ffprobe` on `PATH`. HyperFrames templates additionally require Node.js/npm: the 18 `ref-` templates pin HyperFrames `0.8.29`, and `nine-grid-reveal` pins `0.8.33`.
...
- BGM should follow the overall content and emotional arc, not a single keyword. Use crossfade looping, short fades, and conservative loudness. Enable ducking when narration exists. For `nine-grid-reveal`, preserve the bound audio bytes, timing, volume, and existing fades instead of applying these automatic music treatments.
