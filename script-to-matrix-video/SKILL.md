...
description: Create publishable multi-platform 9:16 Chinese lead-generation MP4s through two independent functions, defaulting to `text-media-text` template video when the user does not name a mode. Template video can use the current Huangque hosted service through HQ CLI with approved platform-library media and optional public or personal voiceover, or the bundled local renderer; AI-generated template media is forbidden. Use full script-to-video only for a complete narrated or storyboarded production. Use for 文案一键成片、模板成片、模板配音、上文字中素材下文字、批量矩阵视频、素材库自动剪辑、AI 图片口播视频 or 矩阵引流视频. Do not use for manual frame-accurate editing of supplied footage.
---
...
Use when the user explicitly asks for 文案一键成片、完整口播分镜、语义分镜、知识讲解完整版, speech-synced subtitles, or a complete client script preserved as a narrated/storyboarded production. Analyze the full context, split it into semantic scenes, retrieve or generate scene material, optionally synthesize narration, add subtitles, motion, transitions, SFX, BGM, and a dedicated first-frame cover. A request explicitly described as 模板成片 remains Function 2 even when it includes voiceover.
...
Use by default for an unspecified “make a video” request, as well as when the user asks for 模板成片、模板配音、上面文字中间素材下面文字, a fixed-title information card, or batch variants of that format. This function has its own inputs, workflow, and deliverables. It does not require semantic storyboarding across a long article; the hosted route may add one continuous voiceover whose duration controls the final video.
...
- Optional input: execution route, background context, asset keywords, preferred image/video mix, BGM preference, duration, variant count, platform, brand style, one `template_id`, and hosted voiceover text/voice/speed.
- Bundled local default behavior: use `layout.template_id: black-left-bold` for the 1080x1920 default style: no narration, fixed top and bottom text, central client-supplied or approved-library media, pure black background, left-aligned bold title, about 5% visible top margin, no divider or decoration, and auto BGM unless the user asks for silence. The Skill bundles 26 approved templates in two groups: eight standard FFmpeg templates (`black-left-bold`, `white-center-bold`, `white-handwritten`, `black-playful`, `white-left-editorial`, `black-right-modern`, `white-left-playful`, and `black-center-editorial`) plus 18 HyperFrames reference-typography templates whose IDs begin with `ref-`. Read [the style template catalog](references/style-templates.md) and [the 18-template reference pack](references/reference-typography-templates.md) when the user asks to choose or compare local styles. Hosted template IDs always come from the live CLI catalog. Do not invent or silently restore a removed template. For AI-selected keyword treatment, read [the semantic emphasis contract](references/semantic-emphasis.md), analyze each copy once, and reuse that neutral result across templates. Never generate AI media for this function. Enforce an 8-second hard minimum; calculate longer standard-template copy from reading time instead of defaulting every output to 8 seconds. For every local `ref-` output, do not ask for or accept a duration: randomly assign one integer from 8 through 15 seconds during job preparation, record it in `prepared-rows.json`, and reuse it when the same task work directory is rendered again. Use three distinct approved video assets so every randomized duration satisfies the material policy.
...
Routing rule: Function 2 is the fallback whenever the mode is absent or ambiguous, including short requests such as “出个视频”, topic-only requests, headline/CTA copy, screenshots, tables, ordinary matrix-video requests, and an explicit template-video request with voiceover. Select Function 1 only when the user explicitly requests full-script production, semantic storyboarding, speech-synced subtitles, or preservation of a long script as a complete narrated video. An explicit mode always wins. Never switch a Function 2 job to Function 1 merely because library media is missing.

## Function 2 execution routes

- **Hosted HQ CLI (preferred for current website behavior):** use when `hq` 0.15.4 or newer is installed and authenticated, or when the user asks for the current website templates, platform material library, public voice, personal cloned voice, speech rate, or account task history. Read [the hosted template CLI contract](references/hosted-template-cli.md) before quoting or submitting. This route is a paid account operation: catalog and capability reads are free, then quote first and wait for explicit confirmation before the paid call.
- **Bundled local renderer:** use when the user explicitly requests offline/local rendering, supplied local media, or template-source customization, or when hosted readiness fails before any quote is created. Continue using the bundled 8 FFmpeg and 18 HyperFrames templates and the local material-library workflow below.
- An explicit route wins. After a hosted quote, submission, or returned `job_id`, never silently switch to local rendering or create a replacement job. Preserve the exact input, quote token, job IDs, and recover the original operation.
...
- For Function 1, prefer client-supplied assets, then semantically relevant library records whose `状态` is `可使用`, then AI-generated images as fallback. For Function 2, stop after client and approved-library assets; AI generation is forbidden. The hosted service selects from its approved platform library; the local route copies selected library files into the project before rendering.
...
- In Function 1, use Alibaba Cloud Bailian CosyVoice for Chinese narration by default. If the user asks for no narration, set `voice.enabled` to `false`, give every scene an explicit reading-duration, and render a silent AAC compatibility track without calling TTS. Function 2 defaults to no narration, but its hosted route supports one optional public or personal cloned voiceover.
...
- Set BGM to `auto` when a configured library has a suitable track and the user has not requested silence. An explicit no-BGM instruction always wins. Hosted Function 2 voiceover always disables BGM instead of ducking it; without hosted voiceover, the service manages its normal BGM behavior. For local narration, keep music subordinate and enable ducking.
...
- Function 1 uses full-frame media by default. Hosted Function 2 selects `template_id` from the live `matrix-template-templates` result. Local Function 2 uses `layout.template_id: black-left-bold`, backed by the `text-media-text` layout in [the layout template reference](references/layout-templates.md), unless the user explicitly selects another bundled template.
- Return only the final MP4 to the user unless they request intermediate artifacts.

## Runtime prerequisites

- Hosted Function 2 requires HQ CLI 0.15.4 or newer plus an authenticated account. Run `hq version --json`, `hq status --json`, `hq run matrix-template-capability --json`, and `hq run matrix-template-templates --json`; read `hq run voices --json` only when voiceover is requested. These reads do not submit a paid task.
- The bundled local renderer requires a verified material-library connection. Before its first render, read [the installation reference](references/installation.md), run `python scripts/material_library.py inspect`, and stop before project creation if it fails. Also run `python scripts/check_environment.py`; add `--require-tts` only for local Alibaba narration.
- Rendering requires Python 3.10 or newer plus `ffmpeg` and `ffprobe` on `PATH`. The 18 `ref-` templates additionally require Node.js/npm; their wrapper pins HyperFrames `0.8.17`.
...
- Read [the material-library reference](references/material-library.md) only for local rendering. The hosted route uses the account's platform library and must not request local SSH credentials. Local remote access must use an existing SSH key or agent; never put a server password in this Skill, a project, a command, or an archive.
...
- In local Function 2, if no contextually suitable supplied or `可使用` library media exists, mark that output `material_missing` and report which semantic searches were attempted. In hosted Function 2, preserve and report the server error. Do not generate AI media, reuse unrelated filler, switch functions, or create a replacement paid task. Other batch jobs may continue.
...
The job is complete only when every hosted task reaches `done` with a usable result URL, or every local final MP4 exists and media probing confirms the requested canvas, playable H.264 video, AAC audio, and nonzero duration. Download and probe hosted output when the environment permits. For a single output, link the MP4 and briefly state its duration and resolution. For a batch, report every original `job_id`, success/failure count, and available output links; include a ZIP and timing report for local batches. Mention fallbacks only if they materially affected quality.
