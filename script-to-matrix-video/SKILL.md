...
- Default behavior: `native-bold` 1080x1920 layout, no narration, fixed top and bottom text, central client-supplied or approved-library media, no yellow divider, restrained motion, and auto BGM unless the user asks for silence. When the user requests a visual style or wants choices, read [the style template catalog](references/style-templates.md) and select one of its 12 stable `template_id` values. Never generate AI media for this function. Enforce an 8-second hard minimum; calculate longer copy from reading time instead of defaulting every output to 8 seconds. Use multiple distinct assets and include approved video material by default.
