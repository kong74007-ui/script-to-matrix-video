...
`text-media-text` uses the renderer's `native-bold` structural variant, while the bundled visual choice is controlled by one of four catalog IDs: `black-left-bold`, `white-center-bold`, `white-handwritten`, or `black-playful`. The default is `black-left-bold`. All four use a solid black or white background, a block title, about 5% visible top margin, central media, a fixed CTA, and no decorative surfaces or text fade-in. Projects may override safe sizing and crop fields without creating an undeclared fifth template. Scene highlight arrays accept strings or objects shaped as `{"text":"40%","color":"#FFD400"}`.
