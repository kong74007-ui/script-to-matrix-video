...
- 1080×1920、30 fps，全屏真实素材，不加黑边、圆角或额外装饰。
...
- 使用用户配音；默认不加 BGM，无绑定曲目，不保留参考原声。长度跟随实测音频，默认向上对齐30fps并留约0.6秒尾帧，不随机8–15秒，不拉伸或截短配音。
...
4. 准备至少三条不同的真实 H.264 视频，建议720×1280或1080×1920、30fps、1秒GOP、SAR1:1、Rec.709 SDR。HLG/PQ先做真实 SDR转换再使用，不能只改标签；保持方向及比例，勿加白色滤层。每条须覆盖自己的 `end-start+offset` 源时长。长配音可加素材，不循环单素材填满。
...
以上仅展示字段，`cues` 必须替换成整段音频的真实完整时间轴。结尾 `end:"end"` 表示保持到片尾。可指定整体 `duration`（30fps帧边界，且不短于音频）；否则按配音自动计算。`media` 默认均分视频长度并重叠0.18秒；为贴合语义可为每项指定 `start/end/offset`，必须首尾连续且每次重叠0.18秒。标题 `style` 只有 `main/subtitle`，不能同位置持续重叠。超长标题/字幕先按语义缩短、拆句，不缩放压扁字体。
...
npm run preview
# 确认后
npm run render -- --quality delivery --fps 30 --workers 2 --strict --output renders/final.mp4
```

准备脚本只复制明确选择的文件，不负责联网转写、生成素材或公开上传。母版共享字体及OFL许可证会被复制进新项目；不要只单独下载模板子目录。用户提供配音完整保留，库素材原声静音。Windows 子进程保持隐藏。
