...
v1.8.0 将当前确认的模板集整理为 8 套标准 FFmpeg 模板和 17 套 HyperFrames 参考排版模板，当时默认是现已删除的 `black-left-bold`。新增参考模板稳定 ID、五层文字输入、批量渲染脚本、17 条 MP4 案例和 17 张首帧 JPG。
...
安装包中的 `assets/examples/text-media-text` 包含 18 套参考排版案例成片和首帧预览，`assets/fonts` 包含模板运行所需字体，因此 v1.8.x 压缩包体积会明显大于旧版。

> 当前离线包已删除原 8 个标准预设，保留 21 个模板（18 ref + 3 动效），默认 ref-15-tianjin-monochrome。下方版本历史不代表当前可选目录。
...
覆盖旧版本前，安装器会先校验标准目录为空、18 套参考模板、18 组案例文件、5 个字体文件、许可证和 SHA-256；分享包不完整时会停止，不会先移动现有 Skill。
