...
安装包中的 `assets/examples/text-media-text` 包含公开案例成片，`assets/fonts` 包含模板运行所需字体，因此 v1.7.0 压缩包体积会明显大于旧版。
...
覆盖旧版本前，安装器会先校验 12 套模板目录、4 个字体文件、许可证和 SHA-256；分享包不完整时会停止，不会先移动现有 Skill。
