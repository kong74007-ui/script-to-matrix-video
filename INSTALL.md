...
v1.8.2 新增美业粉白私域运营参考模板，模板总数更新为 26 套，并补齐案例与安装校验。
...
- 使用 18 套 `ref-` 参考排版模板时，需要 Node.js 和 npm；渲染脚本固定调用 HyperFrames `0.8.16`；
...
覆盖旧版本前，安装器会先校验 8 套标准模板、18 套参考模板、26 组案例文件、4 个字体文件、许可证和 SHA-256；分享包不完整时会停止，不会先移动现有 Skill。
