...
v1.6.0 修复跨电脑运行时容易走最简路径的问题：渲染器会按文案自动计算完整阅读时长，不再把所有视频固定成 8 秒；模板按时长强制使用 2–4 个不同素材并默认至少包含 1 个视频；批量渲染前强制校验 A/B 素材差异和 BGM 轮换。12 条案例、素材库个人连接和“模板成片禁止 AI 生成”规则保持不变。
